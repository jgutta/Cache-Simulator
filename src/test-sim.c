...
int soln_get_block(Cache* cache, address_type address);
...
void test_cache() {
  ...
  test(blocks == cache->block_bits,
       "cache->block_bits properly initialized",
       msg("cache->block_bits improperly initialized: expected %d", blocks),
       3,
       4);
  ...
  test(block_count == cache->block_count,
       "cache->block_count properly initialized",
       msg("cache->block_count impropertly initialized: expected %d", block_count),
       6,
       3);
  ...
  for (int i = 0; i < cache->set_count; i++) {
    ...
    for (int j = 0; j < cache->line_count; j++) {
      testval |= (lines[j].blocks == NULL || lines[j].block_count != block_count);
    }
  }

  test(!testval,
       "blocks and block_count in Line is properly initialized",
       "blocks and block_count in Line is not properly initialized",
       12,
       2);
}
...
void test_bits() {
  ...
  test(b == sb,
      "get_block is returning the proper block index",
      msg("get_block is not returning the proper block index; returned %d; expected %d",
          b, sb),
       15,
      2);
}
